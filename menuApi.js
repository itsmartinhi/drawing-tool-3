...
menuApi.createMenu = (id) => {
    return new Menu(id);
};
menuApi.createItem = (text, callback) => {
    return new MenuItem(text, callback);
};
menuApi.createSeperator = () => {
    return new Seperator();
};
menuApi.createRadioSelect = (text, options, selection, callback) => {
    return new RadioSelect(text, options, selection, callback);
};
class Menu {
    constructor(id) {
        this._getMenusContainer = () => {
        ...
        this.id = id !== null && id !== void 0 ? id : Date.now() * Math.random(); // generate a unique id
        this.items = [];
    }
}
class MenuItem {
...
class RadioSelect {
    constructor(text, options, callback, selection) {
        this.text = text;
...
const initContextMenu = (canvas) => {
    ...
    const outlineColorOptions = new Map();
    ...
    const fillColorOptions = new Map(outlineColorOptions);
    fillColorOptions.set("Transparent", Color.TRANSPARENT);
    // add radio selections
    items.push(menuApi.createRadioSelect("Outline Color", outlineColorOptions, (value) => {
        canvas.setOutlineColorForSelectedShapes(value, true);
    }, "Blue"));
    items.push(menuApi.createSeperator());
    items.push(menuApi.createRadioSelect("Fill Color", fillColorOptions, (value) => {
